...
func TestUpdateUser(t *testing.T) {
	...
	expectedSQL := `UPDATE users SET first_name = \$1 WHERE id = \$2 RETURNING id, first_name, last_name, user_name, email, password, created_at`
	...
	tests := []struct {
	...
	}{
		{
			...
			setupMock: func(mock sqlmock.Sqlmock, req *proto.UpdateUserRequest) {
				rows := sqlmock.NewRows([]string{
					"id", "first_name", "last_name", "user_name", "email", "password", "created_at",
		...
		},
		{
			...
			setupMock: func(mock sqlmock.Sqlmock, req *proto.UpdateUserRequest) {
				expectedDuplicateSQL := `UPDATE users SET user_name = \$1 WHERE id = \$2 RETURNING id, first_name, last_name, user_name, email, password, created_at`
